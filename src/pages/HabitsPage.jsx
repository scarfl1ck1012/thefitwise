...
const BADGES = [
    { name: "First Steps", icon: "🏃", description: "Log your first workout", condition: (xp) => xp >= 50 },
    ...
    { name: "Workout Warrior", icon: "⚔️", description: "Complete 10 workouts", condition: () => false },
...
export default function HabitsPage() {
    ...
    const allBadges = BADGES.map((b) => ({
        ...b,
        earned: earnedBadges.includes(b.name) || b.condition(xp, streak),
    }));
    return (<div className="space-y-6">
        ...
        <Card className="shadow-elevated overflow-hidden">
          <div className="gradient-primary p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-4xl font-bold text-primary-foreground">{xp}</p>
              </div>
              <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 2, repeat: Infinity }} className="bg-primary-foreground/20 p-4 rounded-2xl">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-muted-foreground">{xpInLevel}/500 XP</span>
            </div>
            <Progress value={(xpInLevel / 500) * 100} className="h-3"/>
            <p className="text-xs text-muted-foreground mt-2">{xpToNext} XP to Level {level + 1}</p>
          </CardContent>
        </Card>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
          <Card className="shadow-card">
            <CardContent className="p-4 text-center">
              <motion.div animate={streak > 0 ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 1.5, repeat: Infinity }}>
                <Flame className={`h-8 w-8 mx-auto mb-2 ${streak > 0 ? "text-accent" : "text-muted-foreground"}`}/>
              </motion.div>
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {allBadges.map((badge, i) => (<motion.div key={badge.name} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className={`text-center p-3 rounded-xl border transition-all ${badge.earned
                ? "bg-primary/5 border-primary/20 shadow-card"
                : "bg-muted/30 border-border opacity-50"}`}>
                <motion.span className="text-2xl block mb-1" animate={badge.earned ? { y: [0, -3, 0] } : {}} transition={{ duration: 2, repeat: Infinity, delay: i * 0.3 }}>
                  {badge.icon}
                </motion.span>
      ...
      <Card className="shadow-card">
        ...
        ].map((item) => (<div key={item.action} className="flex justify-between p-2 rounded-lg bg-muted/50">
